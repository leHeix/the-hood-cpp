...
BitStream bs{ &packet->data[0], (packet->length - 1), false };
...
if (server::player_pool.Exists(packet->playerIndex))
{
	...
	if (packet->data != bs.GetData());
	{
		...
		packet->playerIndex = player->PlayerId();
		packet->playerId = RakServer->GetPlayerIDFromIndex(player->PlayerId());
